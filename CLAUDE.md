...
- **Delete** → moved to `projects/.deleted/<name>` (root-only `0700`), stamped with a `.deleted_at` marker. `undelete_project()` restores it. `purge_expired(days=RETENTION_DAYS)` permanently `rmtree`s entries older than `days` (default 90) and drops their groups; runs on startup in `run.py` (default 90) and via `--purge [DAYS]` (cron).
- **Deactivate** → moved to `projects/.deactivated/<name>` via `deactivate_project()`/`reactivate_project()`. Purpose: declutter the listing when there are thousands of projects. No retention — kept until reactivated. `deactivate_inactive(days)` (CLI `--deactivate N`, for cron) auto-deactivates active projects whose most recent file activity — `max(atime, mtime)` over content, **not** ctime — is older than N days.
- **Lock** → **read-only in place** (NOT moved): `lock_project()` writes a root-only `.locked` marker and drops the group-write bit on every content folder (`all` + subfolders, `2770`→`2750`, and the default ACL's write bit), so members keep read access but can't change anything. `unlock_project()` restores `2770` + group rw on files + the default ACL. The gatekeeper root stays `2750` throughout. Implemented with mode bits (`_lock_tree`/`_unlock_tree`), deliberately *not* per-member deny ACLs.

`get_project()` is location-aware (active/`.deleted`/`.deactivated`) and sets `Project.state` (`active`|`deleted`|`deactivated`), `Project.locked` (from the marker) and `days_left`. The detail page shows Lock+Deactivate+Delete on an active unlocked project, Unlock when locked, Reactivate when deactivated, Undelete when deleted; editing is gated on `can_manage and state=='active' and not locked`. The dashboard lists the user's held projects (`held_projects_for()` = deleted + deactivated) in "Deleted"/"Deactivated" sections, and badges active-but-locked projects "read-only".
...
This is the part that requires care — it is the whole point of the app. **Access** is gated by **standard UNIX groups + SetGID bits**. Visibility policy: **whole projects are hidden from non-members** (the `2750` root denies `other`, so Samba ABE hides projects a user has no access to), but **subfolders stay visible to all project members** — even a restricted one shows in the listing (members can't read its contents, just see it exists). Get these modes exactly right:
...
- **Shared folder** `/projects/<name>/all`: `chown root:grp-<name>`, `chmod 2770`. Full collaborative read/write for the whole project group — named `all` to make it obvious every member has access. Created on day one. (Projects created before the rename have `shr`; the code still recognizes both.)
- **Sibling subfolders** (e.g. `mkt`, `samples`) added later, `chmod 2770`. Two flavours, decided by whether members are given (`_assign_subfolder_group` in `app/system.py`): **open** (no members) → owned by the project group `grp-<id>`, so the whole project has read/write (like a second `all`); **restricted** (members listed) → owned by a dedicated `grp-<id>-<area>` group. A restricted subfolder is **still visible to every project member** (not ABE-hidden): `_set_subfolder_visibility()` grants the primary group traverse-only (`setfacl -m g:grp-<id>:--x`) so the folder shows and can be entered, but its contents can't be listed/read (everything inside is owned by the dedicated group, others `---`). `set_subfolder_members` toggles between the two and re-groups the folder + contents. `Subfolder.restricted` (from the folder's actual owning group via `dir_group`) drives the UI badge.
- The `2` prefix (SetGID) on every folder is mandatory — it makes new files/dirs inherit the group.
- **Never modify the root or `/all` when adding siblings.** Siblings are deployed side-by-side, leaving existing folders untouched.
